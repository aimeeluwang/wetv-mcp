...
export class WetvDataService {
     ...
     * 获取剧集列表
     */
    getEpisodes(contentId, page = 1, perPage = 20) {
    ...
    getRecommendations(params = {}) {
        ...
        if (genre) {
            ...
            results = results.filter(item => item.genre.some(ig => ig.toLowerCase().includes(g)) ||
                item.genre.some(ig => {
        ...
        if (mood) {
            ...
            if (moodGenres.length > 0) {
                results = results.filter(item => item.genre.some(g => moodGenres.includes(g)));
            }
        }
        // 按评分 × 热度综合排序
